// series.jsx — Series-level workspace: catalogue of comics, create + configure.
...
function statusHue(s) {
  return ({ 'In production': 285, 'Drafting': 175, 'Visual dev': 330, 'Outlining': 60 })[s] || 280;
}

function Series({ series, setSeries, activeId, setActive, onOpen, flash }) {
  const [cfgId, setCfgId] = React.useState(null);
  const [creating, setCreating] = React.useState(false);
  const cfg = series.find(s => s.id === cfgId);
  ...
  return (
    <div className="ww-series">
      ...
      <p className="ww-series-sub">{series.length} comics in this workbench · {series.filter(s => s.status === 'In production').length} in production · {series.reduce((n, s) => n + s.published, 0)} episodes published.</p>

      <div className="ww-seriesgrid">
        {series.map(s => (
          <div key={s.id} className={cx('ww-scard', s.id === activeId && 'is-active')} style={{ '--sh': `oklch(0.7 0.16 ${s.hue})` }}>
        ...
        <button className="ww-scard-new" onClick={() => setCreating(true)}>
          <b>＋</b><span>New series</span>
        </button>
      </div>
    ...
    <div className="ww-modal" onClick={onClose}>
      <div className="ww-modal-card" onClick={e => e.stopPropagation()}>
        <div className="ww-modal-head">
          <div>
            <div className="ww-pv-kicker">Create</div>
            <h2>New series</h2>
          </div>
...
// ---- configure ----
function SeriesDrawer({ s, onClose, flash }) {
  const isEcho = s.id === 'echo';
  const cast = isEcho ? window.CHARACTERS : [];
  const [pal, setPal] = React.useState(s.palette);
  const ROLES = ['Shadow', 'Midtone', 'Key light', 'Accent', 'Tint', 'Tone'];
  const setColor = (i, v) => setPal((p) => p.map((c, j) => j === i ? v : c));
  const removeColor = (i) => setPal((p) => p.filter((_, j) => j !== i));
  const addColor = () => setPal((p) => [...p, '#8892a0']);
  return (
    <React.Fragment>
      ...
      <aside className="ww-drawer">
        ...
        <div className="ww-drawer-body">
          ...
          <div className="ww-cfg-block">
            ...
            <ul className="ww-cfg-canon">
              {s.canon.map((c, i) => <li key={i}>{c}</li>)}
            </ul>
          </div>
          <div className="ww-cfg-block">
            ...
            {cast.length ? (
              <div className="ww-cfg-cast">
                {cast.map(c => <span key={c.id} className="ww-cfg-castchip"><i style={{ background: `oklch(0.6 0.15 ${c.tint})` }} />{c.name}</span>)}
              </div>
