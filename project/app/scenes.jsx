// scenes.jsx — atmospheric placeholder "art" for vertoon panels.
// Pure CSS/gradient/primitive scenes (no illustration). Each scene is built from
// depth layers so the SAME data drives the editor thumbnail and the live parallax
// preview. Parallax is driven by a single CSS var --p (centered, ~ -1..1) on the root.
...
function injectSceneCss() {
  ...
  const s = document.createElement('style'); s.id = 'ww-scene-css'; s.textContent = SCENE_CSS;
  document.head.appendChild(s);
}

// little silhouette skyline as rects
function Skyline({ tone = '#0c1018', n = 9, base = 0.55 }) {
  const bars = [];
  for (let i = 0; i < n; i++) {
...
// little monospace depth tag that rides along with its plane
function DepthTag({ label, d, x, y, tone }) {
...
// Each scene returns its depth layers. depth multiplier set via data-d for parallax.
const SCENES = {
  // Diagnostic scene: clearly separated planes against a FIXED frame so the
  // relative drift between near and far is unmistakable while scrolling.
  parallax_demo: () => (<React.Fragment>
    {/* deepest background — essentially static */}
    <div data-d="0" style={{ inset: '-12%', background: 'radial-gradient(120% 95% at 50% 32%, #14193a, #070811 76%)' }} />
    {/* FAR plane — dim purple dot grid, drifts slowly */}
    ...
    {/* MID plane — soft teal circles */}
    <div data-d="0.95" style={{ inset: 0 }}>
      {[['16%', '34%', 64], ['68%', '24%', 86], ['44%', '66%', 54], ['80%', '60%', 40]].map(([x, y, s], i) =>
        <div key={i} style={{ position: 'absolute', left: x, top: y, width: s, height: s, borderRadius: '50%', background: 'radial-gradient(circle at 38% 32%, rgba(22,214,180,.55), rgba(22,214,180,.10) 70%, transparent)', boxShadow: '0 0 26px rgba(22,214,180,.25)' }} />)}
      <DepthTag label="Mid" d="0.95" x="9%" y="50%" tone="rgba(22,214,180,.95)" />
    </div>
    {/* NEAR plane — bold bright bars, drifts fast */}
    ...
    {/* FIXED reference frame + reticle (no data-d → does not move) */}
...
// Scene host. py: parallax centered value (~ -1..1). strength: 0..1. loop kind overlay.
function Scene({ kind, py = 0, strength = 0, loop = null, className = '', style = {} }) {
  ...
  React.useEffect(() => {
    ...
    root.querySelectorAll('[data-d]').forEach(el => {
      ...
      // root --p (centered parallax) drives every layer via calc, so an external
      // scroll loop can update one var per panel instead of re-rendering React.
...
Object.assign(window, { Scene, SCENES, injectSceneCss });
